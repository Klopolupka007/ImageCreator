...
- [API-система](#globe_with_meridians-api-система)
...
- выбор модификаторов для генерации изображений;
...
Для запуска модулей необходимо запустить python-файл **use_case_api.py** в директории use_cases.

В папке use_cases находятся исполняемые файлы API для связи сайта с нейронными сетями.
_В common_tools лежат вспомогательные данные._

Подробнее про API в разделе <a href="#globe_with_meridians-api-система">API-система</a>.

### Веб-приложение
Для начала работы с сайтом, необходимо скачать его файлы по ссылке: https://drive.google.com/file/d/1Uw-n9c2aZj76apl9bERb3gKksfO1-sSh/view?usp=sharing

При заходе на сайт высвечивается главное меню с тремя функциями (превью видео, фотография пользователя и баннер канала), а также история пользовательских запросов по этим функциям:
<div align="center"><img src="https://i.imgur.com/xiZz0lY.png"></div>


Страница загрузки видео имеет возможность выбора видеофайла, а также записи названия, описания видео и модификаторов для Stable Diffusion, с помощью которых будет просиходить генерация превью:
<div align="center"><img src="https://i.imgur.com/gyVvnuc.png"></div>

Во время генерации будет указываться шкала прогресса и выводиться само превью по окончании процесса:
<div align="center"><img src="https://i.imgur.com/5HKC0WM.png"></div>


При выборе функции генерации изображения пользователя (аватарки) появляется следующая страница, на которой можно выбрать тип генерации (pose estimation, face estimation, InPaint). При выборе InPaint пользователю предоставляется возможность обвести и выделить область, которую предполагается заменить и соответсвующий промпт замены:
...
<div align="center"><img src="https://i.imgur.com/EOv4Hs3.png"></div>

В данном случае можно выбрать тип генерации по телу (pose), тогда результат получается следующий:
<div align="center"><img src="https://i.imgur.com/slW66Mb.png"></div>
...
Система представляет из себя микросервисную архитектуру, состояющую из двух модулей:
...
## :globe_with_meridians: API-система
В данном разделе представлены ссылки на все развернутые API сервисы, необходимые для функциональной работы AI модуля:
***(Внимание: API с нейросетями для работы с графикой работают только на Linux, остальные API работают локлаьно.)***
* API для запуска Stable Diffusion: [Stable Diffusion API][Stable-Diffusion-API-url]
* API для запуска системы масштабирования изображений (upscale): [Upscale API][Upscale-API-url]
* API для запуска автоматического сбора описаний видео: [Video Description][video-description-url]


<p align="right">(<a href="#image-creator">Наверх</a>)</p>

## :memo: Генерация промпта
Для установки модели Llama необходимо скачать ее по ссылке https://huggingface.co/IlyaGusev/saiga2_7b_gguf/resolve/main/model-q8_0.gguf

***ВАЖНО: модель должна лежать в одной папке с requirements.txt***
...
* Панкрухин Максим Сергеевич (CV/PHP) - https://t.me/Klopolupka
* Галяутдинов Аскар Тимурович (NLP) - Project Manager - asckar.ivanov@yandex.ru - https://t.me/askarka7
